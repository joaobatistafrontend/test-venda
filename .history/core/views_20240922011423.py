from typing import Any
from django.http import HttpRequest
from django.http.response import HttpResponse as HttpResponse
from django.shortcuts import render
from .models import *
from django.views.generic import TemplateView

class IndexView(TemplateView):
    template_name = 'index.html'
    def get(self, request):
        produtos = Produto.objects.all()

        return render(request, self.template_name, {'produtos':produtos})
    


def adicionar_ao_carrinho(request, produto_id, empresa_slug):
    if request.method == 'POST':
        empresa = get_object_or_404(Empresa, slug=empresa_slug)
        produto = get_object_or_404(Produto, pk=produto_id, empresa=empresa, disponivel=True)

        # Verifica se já existe um item do carrinho para este produto
        item, created = ItemDoCarrinho.objects.get_or_create(empresa=empresa, produto=produto, defaults={'quantidade': 0})

        # Se o item já existe, incrementa a quantidade
        if not created:
            item.quantidade += 1
        else:
            item.quantidade = 1  # Se o item foi criado agora, inicializa a quantidade como 1

        # Calcula o preço total do item no carrinho com base no valor do produto
        item.preco_total = item.quantidade * produto.valor
        item.save()

        messages.success(request, f'{produto.nome_produto} foi adicionado ao seu carrinho.')

        return redirect('index', empresa_slug=empresa.slug)
    else:
        return redirect('index')  # Redireciona de volta para a página inicial se o método não for POST