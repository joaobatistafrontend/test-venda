...
class IndexView(TemplateView):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, {'produtos':produtos, 'carinho': carinho, 'vendas': vendas})
...
def remover_todos_do_carrinho(request, produto_id):
    item = get_object_or_404(ItemDoCarrinho, pk=produto_id)
    if item.quantidade >0:
        item.delete()
    return redirect('venda')
